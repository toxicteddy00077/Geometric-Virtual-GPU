...
static inline uint8_t gvg_setpixel(VirtualGPU *restrict gpu,
                                   Instruction instr) {
...
static inline uint8_t gvg_getpixel(VirtualGPU *restrict gpu,
                                   Instruction instr) {
  uint_fast32_t dest = INSTR_DEST(instr.fields.addr);
  uint_fast32_t src = INSTR_SRC1(instr.fields.addr);
  ...
  if (x >= gpu->width || y >= gpu->height) {
    gpu->regs[dest].r = gpu->regs[dest].g = gpu->regs[dest].b =
        gpu->regs[dest].a = 0.0f;
    return 0;
  }
...
static inline uint8_t gvg_setdepth(VirtualGPU *restrict gpu,
                                   Instruction instr) {
  uint_fast32_t dest = INSTR_DEST(instr.fields.addr);
  uint_fast32_t src = INSTR_SRC1(instr.fields.addr);

  if (!gpu->depthbuffer || dest >= 16 || src >= 16)
    return 1;
  ...
  uint_fast32_t offset = y * gpu->width + x;
  gpu->depthbuffer[offset] = gpu->regs[src].x;

  return 0;
}

static inline uint8_t gvg_getdepth(VirtualGPU *restrict gpu,
                                   Instruction instr) {
  uint_fast32_t dest = INSTR_DEST(instr.fields.addr);
  uint_fast32_t src = INSTR_SRC1(instr.fields.addr);
  ...
  if (x >= gpu->width || y >= gpu->height) {
    gpu->regs[dest].x = 0.0f;
    return 0;
  }

  uint_fast32_t offset = y * gpu->width + x;
  gpu->regs[dest].x = gpu->depthbuffer[offset];
  gpu->regs[dest].y = 0.0f;
  gpu->regs[dest].z = 0.0f;
  gpu->regs[dest].w = 0.0f;
...
uint8_t operation_map(VirtualGPU *restrict gpu) {
  ...
  case GVG_GETPIXEL:
    ...
    break;
  case GVG_SETDEPTH:
    status = gvg_setdepth(gpu, instr);
    break;
  case GVG_GETDEPTH:
    status = gvg_getdepth(gpu, instr);
    break;
  default:
